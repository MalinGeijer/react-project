...
# Delete all existing rows in the products table
...
def insert_product(image_url, name, brand, price, description):
    """Add a new product with the given image URL to the database."""
    conn = sqlite3.connect("products.db")
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO products (media_url, name, brand, price, description)
        VALUES (?, ?, ?, ?, ?)
    """, (image_url, name, brand, price, description))
    conn.commit()
    conn.close()
...
for item in items:
    ...
    if media_url:
        insert_product(media_url, extract_name(caption), "Craft Etc", 300, "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum vitae ligula eu libero pulvinar posuere. Praesent ac sapien ut velit tincidunt fermentum.")

print(f"Database 'products.db' created.")
